      ...
      <div className="story-card">
        ...
        <div className="story-card__content">
          <h2 className="story-card__tilte">
            <Link to={`/truyen/${data.url}`}>
              {data["tentruyen"] || <Skeleton />}
            </Link>
          </h2>
          <div className="story-card__rate">
            {data.danhgia ? (
              <>
                <span className="story-card__view">{`${data.luotdoc} lượt đọc`}</span>
              </>
            ) : (
              <Skeleton />
            )}
          </div>
          <div className="story-card__description text-secondary">
            {data.noidung || <Skeleton count={2} />}
          </div>
