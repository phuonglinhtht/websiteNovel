...
const apiMain = {
  ...
  getStoriesByUsername: async (params) => {
  ...
  getChapterByNumberAndSetReading: async (
    tentruyen,
    chapnum,
    user,
    dispatch,
  ...
  updateNovelView: async (param) => {
    return getData(await axiosClient.put("/novels/novel/update-view", param));
  },
  ///Comment
  ...
  checkSaved: async (user, dispatch, stateSuccess, params) => {
    const axi = await axiosInstance(user, dispatch, stateSuccess);
    return getData(
      await axi.get(`/saved/${params.url}`, {
        headers: { Authorization: `Bearer ${user.accessToken}` },
      })
    );
  },
