...
export const ListChapter = props => {
  ...
  return (
    <>
      <h3 className='mb-1'>Danh sách chương</h3>
      {
        loadingData ? <LoadingData /> :
          <Grid gap={15} col={props.col || 1} smCol={1}>
            
              {
              chapters.map((item, index) => {
                return <Link to={`/truyen/${url}/${item.chapnumber}`}
                  key={index} className='text-overflow-1-lines d-block'
      ...
      <Pagination totalPage={Math.round(props.totalPage/size)} currentPage={currentPage} handleSetPage={setCurrentPage} />
