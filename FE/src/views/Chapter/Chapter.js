...
function Chapter(props) {
  ...
  const [hasDoneRead, setDoneRead] = useState(false);
  ...
  useEffect(() => {
    const getChapter = async () => {
      if (user) {
        apiMain
          .getChapterByNumberAndSetReading(
            url,
            chapnum,
            user,
            dispatch,
  ...
  useEffect(() => {
    ...
    const handleScrollContent = () => {
      const isAtBottom =
        window.scrollY + 1000 >= contentRef.current.clientHeight &&
        !!chapter.content;
  ...
  useEffect(async () => {
    if (hasDoneRead) {
      try {
        await apiMain.updateNovelView({ url: url });
      } catch (err) {
        console.error(err);
      }
    }
  ...
  const onClickToggleManual = () => {
    ...
    if (list) {
      ...
      else icon.classList.replace("bx-arrow-from-bottom", "bx-arrow-from-top");
    }
  };

  const onClickNextChap = () => {
    if (truyen && Number(chapnum) < truyen.sochap) {
      ...
      >
        <div className="container" style={{ paddingBottom: "3rem" }}>
            ...
            <div className="d-lex">
              ...
              <ul className="chapter__info">
                <li className="text-with-icon">
                  ...
                  {chapter?.tentruyen || "Tên truyện"}
                </li>
                <li className="text-with-icon">
                  <i className="bx bx-edit"></i>
                  {chapter?.nguoidangtruyen || "Người đăng"}
                </li>
                <li className="text-with-icon">
                  <i className="bx bx-text"></i>
                  {chapter?.content?.split(" ").length || 0} chữ
                </li>
                <li className="text-with-icon">
                  <i className="bx bx-time"></i>
                  {chapter?.createAt || Date()}
                </li>
              </ul>
              <div
              ...
              >
                ...
                {chapter?.content ? "" : <Skeleton count={20} />}
              </div>
            </div>
          </div>
          <div className="chapter__nav">
            <div>
              <span className="text-with-icon" onClick={onClickPreviousChap}>
                <i className="bx bx-arrow-back"></i>Chương trước
              </span>
            </div>
            <div>
              <span className="text-with-icon" onClick={onClickNextChap}>
                Chương sau
                <i className="bx bx-arrow-back bx-flip-horizontal"></i>
              </span>
            </div>
          ...
          <div className="chapter-manual fs-24" style={styleManual}>
            <span
              onClick={onClickToggleManual}
              className="chapter-manual__item chapter-manual__item--dropdown"
            >
              <i id="icon-manual" className="bx bx-arrow-from-top"></i>
            </span>
            <ul className="chapter-manual__list" id="chapter-manual__list">
              <li
                className={`chapter-manual__item ${
                  manual === "list-chap" ? "active" : ""
                }`}
                onClick={(e) => {
                  e.stopPropagation();
                  if (manual === "list-chap") setManual("");
                  else setManual("list-chap");
                }}
              >
                <span>
              ...
              <li
                className={`chapter-manual__item ${
                  manual === "setting" ? "active" : ""
                }`}
                onClick={(e) => {
                  e.stopPropagation();
                  if (manual === "setting") setManual("");
                  else setManual("setting");
                }}
              >
                <span>
                          ...
                          <td className="col-8">
                            <div className="d-flex chapter-setting__input">
                              <button
                                onClick={() => {
                                  setLineHeight((pre) => {
                                    return Number((pre - 0.1).toFixed(1));
                                  });
                                }}
                              >
                                <i className="bx bx-minus"></i>
