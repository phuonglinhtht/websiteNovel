...
export const NovelController = {
  CreateNovel: async (req, res) => {
    try {
      ...
      const nguoidangtruyen = new mongoose.Types.ObjectId(
        req.body.nguoidangtruyen
      );
      const novel = await new Novel({
        tentruyen,
        url,
        hinhanh,
        theloai,
      ...
      return res
        .status(400)
        .json(ResponseDetail(400, { message: "Đăng truyện không thành công" }));
    } catch (error) {
      console.log(error);
      return res
        .status(500)
    ...
    try {
      ...
      if (!newUser)
        return req.status(405).json(
          ResponseDetail(403, {
            message: "Bạn không có quyền sửa truyện của người khác",
          })
        );

      const novel = await Novel.findOne({
        _id: id,
        nguoidangtruyen: newUser.id,
      });
      if (!novel)
        return res.status(400).json(
          ResponseDetail(400, {
      ...
      if (newNovel) return res.status(200).json(ResponseData(200, novel));
      return res
        .status(400)
        .json(ResponseDetail(400, { message: "Sửa truyện không thành công" }));
    } catch (error) {
      console.log(error);
      return res
        .status(500)
    ...
    try {
      ...
      if (!newUser)
        return res.status(405).json(
          ResponseDetail(403, {
            message: "Bạn không có quyền xoá truyện của người khác",
          })
        );
      ...
      if (novel) {
        ...
        if (response.deletedCount == 1)
          return res
            .status(200)
            .json(ResponseData(200, { message: "Xoá truyện thành công" }));
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Xoá truyện không thành công" })
          );
      } else
        return res
          .status(400)
      ...
      return res
        .status(500)
        .json(ResponseDetail(500, { message: "Lỗi sửa truyện" }));
    }
  },
    ...
    try {
      ...
      if (content.length <= 10)
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Nội dung phải dài hơn 10 kí tự" })
          );
      const novel = await Novel.findOne({ url: url });
      if (novel) {
        const newestChap = await Chapter.find({ dautruyenId: novel._id })
          .sort({ chapnumber: -1 })
          .limit(1);
        let chapnumber = 1;
        ...
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Đăng chương không thành công" })
          );
      }
      return res
        .status(400)
  ...
  UpdateChapter: async (req, res) => {
    try {
      ...
      if (novel) {
        ...
        const newChap = await Chapter.findOneAndUpdate(
          { chapnumber, dautruyenId: novel.id },
          { content, tenchap },
          { new: true }
        );
        ...
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Sửa chương không thành công" })
          );
      }
      return res
        .status(400)
  ...
  DeleteChapter: async (req, res) => {
    try {
      ...
      if (novel) {
        ...
        const newChap = await Chapter.findOneAndDelete({
          chapnumber,
        ...
        if (newChap)
          return res
            .status(200)
            .json(ResponseData(200, { message: "Xoá chương thành công" }));
        return res
          .status(400)
          .json(
            ResponseDetail(400, { message: "Xoá chương không thành công" })
          );
      }
      return res
        .status(400)
  ...
  GetNovelsByUserId: async (req, res) => {
    try {
      ...
      let novels = await Novel.find({ "nguoidangtruyen.id": user._id })
        .limit(size)
        .skip(size * page)
        .sort({ tentruyen: -1 });
      console.log("novels: ", novels);
      res.status(200).json(ResponseData(200, novels));
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      Novel.findOne({ url: url })
        ...
        .catch((err) => {
          ...
          res
            .status(400)
            .json(ResponseDetail(500, { message: "Không tìm thấy truyện" }));
        });
    } catch (error) {
      console.log(error);
      res
      ...
      if (novel) {
        Chapter.findOne({ dautruyenId: novel.id, chapnumber: chapNumber })
          ...
          .catch((err) => {
            ...
            return res
              .status(400)
              .json(ResponseDetail(500, { message: "Không tìm thấy chap" }));
          });
      } else {
        return res
          .status(400)
  ...
  SetReading: async (req, res) => {
    try {
      ...
      if (!decode.sub) {
        return res
          .status(500)
          .json(ResponseDetail(500, { message: "Lỗi token" }));
      }
      ...
      User.findOne({ username: username })
        .then(async (result) => {
          ...
          return res
            .status(500)
            .json(ResponseDetail(500, { message: "Không tìm thấy tài khoản" }));
        })
        .catch((err) => {
          return res
            .status(500)
            .json(ResponseDetail(500, { message: "Lỗi tìm tài khoản" }));
        });
    } catch (error) {
      console.log(error);
      return res
  ...
  GetReadings: async (req, res) => {
    try {
      ...
      if (user) {
        let readings = await Reading.find({ userId: user._id })
          .populate("dautruyenId")
          .populate("userId");

        readings = await Promise.all(
          readings.map(async (item) => {
            let sochap = await Chapter.countDocuments({
              dautruyenId: item.dautruyenId.id,
            });
            return {
    ...
    try {
      ...
      chaps = chaps.map((item) => {
        return {
          ...
          tentruyen: item.dautruyenId.tentruyen,
          tenchap: item.tenchap,
          tacgia: item.dautruyenId.tacgia,
  ...
  GetReadingsDefault: async (req, res) => {
    try {
      ...
      novelReading = await Promise.all(
        novelReading.map(async (item) => {
    ...
    try {
      ...
      if (!novel) {
        return res.status(404).json(
          ResponseData(404, {
            message: `Không tìm thấy truyện có url: ${url}`,
          })
        );
      }

      novel.luotdoc = novel.luotdoc + 1;
      await novel.save();

      return res
        .status(200)
        .json(ResponseData(404, { message: "Cập nhật thành công" }));
    } catch (err) {
      console.log(err);
      return res
        .status(500)
        .json(ResponseDetail(500, { message: "SERVER ERROR" }));
    }
  },
};
